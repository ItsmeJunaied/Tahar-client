...
const ShopMenFilter = ({ ShopMen, activeFabric, setActiveFabric, setFilteredProfuct }) => {
    ...
    const MaxMinPrice = ShopMen.map(item => item.price);
    ...
    const FindFabricsData = ShopMen.filter(item =>
        ChooseFabrics?.includes(item.fabrics)
    );

    //show selected color data
    const FindColordata = ShopMen.filter(item => {
        const colors = [
    ...
    const FindSizedata = ShopMen.filter(item => {
    ...
    const handleApply = () => {
        const FinalResult = ShopMen.filter(item => {
